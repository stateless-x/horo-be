...
function validateStructuredDailyReading(data: Record<string, unknown>): string | null {
...
const STRUCTURED_DAILY_SHAPE = `
Return valid JSON matching exactly this shape (all fields required):
{
  "overallReading": string,
  "categories": {
    "career": { "reading": string, "score": integer, "tip": string },
    "love": { "reading": string, "score": integer, "tip": string },
    "finance": { "reading": string, "score": integer, "tip": string },
    "health": { "reading": string, "score": integer, "tip": string }
  },
  "dos": string[],
  "donts": string[],
  "luckyMoment": string
...
/**
 * Generate a structured daily reading using DeepSeek's JSON mode.
...
export async function generateStructuredDailyReading(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      const validationError = validateStructuredDailyReading(parsed);
      if (validationError) {
        if (!usedValidationRetry) {
          usedValidationRetry = true;
          effectivePrompt = `${effectivePrompt}\n\nYour previous response was invalid: ${validationError}. Return the complete JSON object with all required fields.`;
      ...
      lastError = error instanceof Error ? error : new Error(String(error));
      console.error(
        `[DeepSeek] Structured daily generation attempt ${attempt + 1}/${maxRetries + 1} failed:`,
        lastError.message
      );
